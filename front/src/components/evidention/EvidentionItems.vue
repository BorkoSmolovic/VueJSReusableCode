<template>
  <v-container fluid grid-list-sm>
    ...
    >
      ...
      <!--</template>-->
      <template v-slot:item.name="{ item }">
        <span>{{item.item.name}}</span><span v-if="item.vehicle != null">{{' ('+item.vehicle.type+', '+item.vehicle.plates+') '}}</span>
      </template>
    ...
    computed: {
      headers() {
        return [
          ...
          {text: 'Cijena', name: 'value', align: 'left', sortable: true, value: 'value'},
