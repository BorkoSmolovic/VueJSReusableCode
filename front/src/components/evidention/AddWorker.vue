...
<script>
  import {add, readWorkersActive, readWorkerTypesActive} from "../../statics/DataTableFunctions";
  ...
  export default {
    ...
    methods: {
      ...
      readWorkersActive,
      getWorkersByType() {
          this.form.worker_type_id = this.form.workerType === null ? null : this.form.workerType.id;
          this.form.worker = null
          this.form.filteredWorkers = [];
          for (let i = 0; i < this.workers.length; i++) {
            for (let j = 0; j < this.workers[i].worker_type_worker.length; j++) {
              if (this.form.workerType != null && this.workers[i].worker_type_worker[j].worker_type_id === this.form.workerType.id) {
