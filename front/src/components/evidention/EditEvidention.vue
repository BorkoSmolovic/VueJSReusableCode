                  ...
                  >
                    <template v-slot:item.type="{ item }">
                      <v-combobox
                        ...
                        :items="workerTypes"
                        @change="item.worker_type_id =item.workerType.id && getWorkersByType(item)"
                        :rules="rules.required"
                        :label="'Tip radnika'+'*'"
                        item-text="name"
                        autocomplete="off"
    ...
    watch: {
      editItemDialog: function (val) {
        if (val) {
          ...
          // Ucitavanje tipova usluga
          this.form.services = this.dataForModal.services

          for (let i = 0; i < this.dataForModal.workers.length; i++) {
            this.form.workers.push({
              worker_type_id: this.dataForModal.workers[i].worker_type_id,
              worker_id: this.dataForModal.workers[i].worker_id,
              workerType: this.workerTypes[this.findById(this.dataForModal.workers[i].worker_type_id, this.workerTypes)],
              worker: this.workers[this.findById(this.dataForModal.workers[i].worker_id, this.workers)],
              salary: this.dataForModal.workers[i].salary,
              filteredWorkers: [],
            })
            this.getWorkersByType(this.form.workers[i]);
          }
          if (this.dataForModal.items != null) {
            for (let i = 0; i < this.dataForModal.items.length; i++) {
              this.form.items.push({
                item: this.items[this.findById(this.dataForModal.items[i].item_id, this.items)],
                item_id: this.dataForModal.items[i].item_id,
                value: this.dataForModal.items[i].value,
                vehicle: this.dataForModal.items[i].vehicle,
                vehicle_id: this.dataForModal.items[i].vehicle_id,
                hasVehicle: this.dataForModal.items[i].hasVehicle,
              })
            }
          }
